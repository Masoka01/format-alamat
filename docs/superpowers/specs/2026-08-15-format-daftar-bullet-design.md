...
- Mode huruf (title/upper/lower/none) dan koreksi typo (`fixAddressTypos`) tidak berubah.
...
- **Ubah**: `src/components/Formatter.tsx` — hapus `stripPrefix` (baris 38–42 saat ini), `formatLines` memakai `formatListLine`. Tidak ada perubahan lain.
...
Regression:
- Alamat: `1. omah kampung` → `- Omah Kampung`; `Jl.Raya Sudirman No.5` → `Jl. Raya Sudirman No. 5`; `Komp. Permata Indah RT.01/RW.02` → `Komp. Permata Indah RT. 01/RW. 02`
